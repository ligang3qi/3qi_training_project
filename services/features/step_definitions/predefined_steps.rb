...
def open_with_capabilities browser
	caps          = Selenium::WebDriver::Remote::Capabilities.send browser.to_sym
	caps.version  = @params["version"]
	caps.platform = @params["saucelabs_os"]
	caps[:name]   = "#"+@params["log_properties"]["job_id"].to_s+"-"+@params["log_properties"]["test_case_name"].to_s

	@browser = Watir::Browser.new(:remote, :url => "http://#{@params["saucelabs_username"]}:#{@params["saucelabs_access_key"]}@ondemand.saucelabs.com:80/wd/hub", :desired_capabilities => caps)
end

def open_firefox
	setup_watir_webdriver
	if @params["run_type"].eql?("Saucelab")
		open_with_capabilities("firefox")
	else
...
def open_chrome
	setup_watir_webdriver
	if @params["run_type"].eql?("Saucelab")
		open_with_capabilities("chrome")
	else
		if RUBY_PLATFORM && RUBY_PLATFORM == "java" && @params
			#mac
			shamisen_dir         = "~/awetest/shamisen"
			selenium_server_path = "#{shamisen_dir}/bin/selenium-server-standalone.jar"
			#selenium_server_path = "$HOME/Awetest/shamisen/bin/selenium-server-standalone.jar"
			output               = '> /dev/null 2>&1'
...
def open_internet_explorer
	if @params["run_type"].eql?("Saucelab")
		setup_watir_webdriver
		open_with_capabilities("internet_explorer")
	else
		if $watir_script
			setup_classic_watir
...
Then /^I click the "?(.*?)"? with "([^"]*)" "([^"]*)"$/ do |element, how, what|
	...
	if target.respond_to?("click")
		if $watir_script
			...
			if browser_name == :internet_explorer
				# if ie_click and orig_what =~ /start/i
				#   binding.pry
				# end
